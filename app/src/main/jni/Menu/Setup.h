...
const std::string expectedTitle = "BY LIYA HACK";
...
void Init(JNIEnv *env, jobject thiz, jobject ctx, jobject title, jobject subtitle){
    //Set sub title
    setText(env, title, OBFUSCATE("<b>Modded by @liyaharuno & @CheatCodeRevo </b>"));

    //Set sub title
    setText(env, subtitle, OBFUSCATE("<b><marquee><p style=\"font-size:30\">"
                                     "<p style=\"color:YELLOW;\">Modded by Liya</p> | "
                                     "CheatCode Group @CheatCodeRevo</p>"
                                     "</marquee></b>"));


    //Toast Example
    Toast(env,ctx,OBFUSCATE("MADE BY LIYA HACK @liyaharuno"),ToastLength::LENGTH_LONG);
